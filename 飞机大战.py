...
# 加载红心、蓝色水晶图片
powerup_images = {}
powerup_images['article1'] = pygame.image.load(path.join(img_dir, 'article1.png')).convert()
powerup_images['hero_blood'] = pygame.image.load(path.join(img_dir, 'hero_blood.png')).convert()
...
def main_menu():
    ...
    # 加载游戏初始界面背景图片
    title = pygame.image.load(path.join(img_dir, "主界面.jpg")).convert()
...
class Pow(pygame.sprite.Sprite):
    ...
    def __init__(self, center):
        pygame.sprite.Sprite.__init__(self)
        self.type = random.choice(['article1', 'hero_blood'])
...
while running:
    if menu_display:
        ...
        pygame.mixer.music.load(path.join(sound_folder, 'game_music.ogg'))
        pygame.mixer.music.play(-1)
        menu_display = False
    ...
    for hit in hits:
        if hit.type == 'hero_blood':
            player.shield += random.randrange(10, 30)
            if player.shield >= 100:
                player.shield = 100
        if hit.type == 'article1':
            player.powerup()
